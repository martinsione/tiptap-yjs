...
export class Document extends YServer<CloudflareEnv["Document"]> {
...
export default {
  async fetch(request, env, ctx) {
    const partykit = await routePartykitRequest(
      request,
      env as unknown as Record<string, unknown>,
    );

    return partykit ?? new Response("Not found", { status: 404 });
  },
